...
<div class="flex flex-wrap space-x-8">
    <div class="grow mt-6 space-y-4">
        <x-field.input name="name" label="Name" :readonly="$readonly" value="{{ old('name', $teacher->user->name) }}" />
        ...
        <x-field.radiogroup name="gender" label="Gender" :readonly="$readonly"
            value="{{ old('gender', $teacher->user->gender) }}" :options="['M' => 'Masculine', 'F' => 'Feminine']" />
        <x-field.select name="department" label="Department" :readonly="$readonly"
            value="{{ old('department', $teacher->department) }}" :options="$departments" />
    ...
    <div class="pb-6">
        <x-field.image name="photo_file" label="Photo" width="md" :readonly="$readonly" deleteTitle="Delete Photo"
            :deleteAllow="true" deleteForm="form_to_delete_photo" :imageUrl="$teacher->user->photoFullUrl" />
    </div>
</div>
